...
(test attribute-specification
  (with-optima-frob (attribute-specification)
    ...
    (frob '(:attribute-spec cl-vhdl::max-skew (cl-vhdl::clock-pair) :group (cl-vhdl::ps 200))
	  "attribute max_skew of clock_pair : group is 200 ps;")
    ))

(test group-template-declaration
  (with-optima-frob (group-template-declaration)
    (frob '(:group-template cl-vhdl::signal-pair :signal :signal)
	  "group signal_pair is (signal, signal);")
    (frob '(:group-template cl-vhdl::component-instances (:label :<>))
	  "group component_instances is ( label <> );"))
  (with-optima-frob (group-declaration)
    (frob '(:group cl-vhdl::clock-pair (:compound cl-vhdl::signal-pair)
